...
namespace Bili.SignIn.Desktop
{
    ...
    public partial class AuthorizeProvider
    {
        ...
        internal static async Task<string> GetConfirmUriAsync()
        {
        ...
        internal string GenerateSign(Dictionary<string, string> queryParameters)
        {
            ...
            var apiSecret = apiKey switch
            {
                Keys.IOSKey => Keys.IOSSecret,
                Keys.AndroidKey => Keys.AndroidSecret,
                Keys.LoginKey => Keys.LoginSecret,
                _ => Keys.WebSecret,
            };
        ...
        private static long GetNowSeconds()
            => DateTimeOffset.Now.ToLocalTime().ToUnixTimeSeconds();

        private static long GetNowMilliSeconds()
            => DateTimeOffset.Now.ToLocalTime().ToUnixTimeMilliseconds();
            ...
            catch (ServiceException se)
            {
                ...
                if (se.Error != null)
                {
                    var qrStatus = se.Error.Code == 86039
                        ...
                        : se.Error.Code == 86038 || se.Error.Code == -3 ? QRCodeStatus.Expiried : QRCodeStatus.Failed;
