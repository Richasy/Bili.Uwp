...
namespace Richasy.Bili.App.Controls
{
    internal class FlowLayoutAlgorithm
    {
        ...
        private Rect RealizationRect => IsVirtualizingContext ? _context.RealizationRect : new Rect(0, 0, double.PositiveInfinity, double.PositiveInfinity);
        ...
        {
            var anchorIndex = -1;
            var anchorPosition = default(Point);
            ...
            {
                var isRealizationWindowConnected = _elementManager.IsWindowConnected(RealizationRect, _orientation.ScrollOrientation, _scrollOrientationSameAsFlow);
        ...
        private Rect EstimateExtent(Size availableSize, string layoutId)
        {
            ...
            var firstBounds = default(Rect);
            UIElement lastRealizedElement = null;
            var lastBounds = default(Rect);
            ...
            var extent = _algorithmCallbacks.Algorithm_GetExtent(
                availableSize,
                _context,
        ...
        private void RaiseLineArranged()
        {
            ...
            if (realizationRect.Width != 0.0f || realizationRect.Height != 0.0f)
            {
                ...
                if (realizedElementCount > 0)
                {
                    ...
                    for (var currentDataIndex = _firstRealizedDataIndexInsideRealizationWindow; currentDataIndex <= _lastRealizedDataIndexInsideRealizationWindow; currentDataIndex++)
                    {
            ...
            string layoutId)
        {
            for (var rangeIndex = lineStartIndex; rangeIndex < lineStartIndex + countInLine; ++rangeIndex)
            {
                ...
                bounds.X -= _lastExtent.X;
                bounds.Y -= _lastExtent.Y;
